...
const formData = ref({
  peopleInvolved: [props.studentInfo.name],
...
const validate = () => {
  ...
  if (!formData.value.dateOfIncident) {
    errors.value.dateOfIncident = 'Date is required';
  }
  if (new Date(formData.value.dateOfIncident) > new Date()) {
    errors.value.dateOfIncident = 'Date cannot be in the future';
  }
  if (!formData.value.placeOfIncident?.trim()) {
    errors.value.placeOfIncident = 'Place of incident is required';
  }
  if (!formData.value.thingsInvolved?.trim()) {
    errors.value.thingsInvolved = 'Things involved is required';
  }
  if (!formData.value.narrativeReport?.trim()) {
    errors.value.narrativeReport = 'Narrative report is required';
  }
  if (!formData.value.actionTaken?.trim()) {
    errors.value.actionTaken = 'Initial action taken is required';
  }
