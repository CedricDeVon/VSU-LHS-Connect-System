export const verifyLogin = (userName, pass) => {
  const index = users.findIndex(
    (usr) => usr.username === userName && usr.password === pass && usr.canAccess
  );
  if (index !== -1) {
    return true;
  }
...
export const createUser = (newUser) => {
  const userExists = users.some(
    (user) => user.emailAdd === newUser.emailAdd || user.username === newUser.username
  );
...
export const users = [
  ...
  {
    userId: "userid8",
    emailAdd: "sample22-1-01134@vsu.edu.ph",
    username: "euj",
    password: "Wangyour",
    canAccess: true,
  },
