...
export default defineEventHandler(async (event: any) => {
    try {
        ...
        let result: any = await Apis.nuxtRestApi.createOne(
            '/api/v1/anecdotal_reports/[anecdotal-id]/[report-id].post', event, client.data, 'anecdotal_report',
            { 'anecdotal-id': getRouterParam(event, 'anecdotal-id'),
                'report-id': getRouterParam(event, 'report-id') }, bodyParameters);
